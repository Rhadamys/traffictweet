...
public interface OccurrenceNodeRepository extends Neo4jRepository<OccurrenceNode, Long> {
    @Query("MATCH (o:Occurrence) WHERE o.millis = {millis} " +
            "MATCH (c:Commune) WHERE (o)-[:REPORTED_AT]->(c) " +
            "MATCH (adj:Commune) WHERE (adj)-[:ADJACENT_TO]->(c) " +
            "MATCH (x:Occurrence) WHERE x <> o AND 0 < o.millis - x.millis <= 10800000 AND " +
